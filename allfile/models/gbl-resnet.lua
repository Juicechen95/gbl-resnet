...
local Gbconv2 = cudnn.Gaborconv2
local bcm = cudnn.BCM
...
local function createModel(opt)
   ...
   if opt.dataset == 'imagenet' then
      ...
      local ngbout = opt.ngbout
      local mode = opt.mode
      model:add(Gbconv2(3,ngbout,7,7,2,2,3,3,mode,0))
      model:add(bcm(ngbout,64,3,3,1,1,1,1))
...
--------------------------------------------------------------------------------------------------------------------------------------------
      ...
      local mode = opt.mode ---theta,gamma,lambda
        if opt.gbconv == 5 then
            model:add(Convolution(3,ngbout,3,3,1,1,1,1))
            model:add(bcm(ngbout,16,3,3,1,1,1,1))
        elseif opt.gbconv == 6 then
            model:add(Convolution(3,ngbout,3,3,1,1,1,1))
            model:add(Convolution(ngbout,16,3,3,1,1,1,1))
        else
            model:add(Gbconv2(3,ngbout,gbsz,gbsz,gbst,gbst,(gbsz-1)/2,(gbsz-1)/2,mode, 0))
            model:add(bcm(ngbout,16,3,3,1,1,1,1))
        end
      --model:add(Convolution(3,16,3,3,1,1,1,1))
      model:add(SBatchNorm(16))
      if opt.gbconv == 4 then
      model:add(LeakyReLU(true))
      else
        model:add(ReLU(true))
      end
...
elseif opt.gbst ==2 then
	  model:add(Avg(4,4,1,1))
else
print('gbsz and Avg of model erro')
end
