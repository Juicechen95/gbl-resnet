...
local Gbconv_lr, parent = torch.class('cudnn.Gbconv_lr', 'cudnn.SpatialConvolution')

function Gbconv_lr:__init(nInputPlane, nOutputPlane, kW, kH, dW, dH, padW, padH, mode, gblr)
  --self.ri = ri
...
function Gbconv_lr:reset()
	...
	elseif self.mode == '5l1' then--gbsz5 l=2
		print('in t1')
		for j = 1, self.nOutputPlane,2 do -- for RGB, filters are the same
			local theta =  (180/self.nOutputPlane)*(j-1)
			print(j)
			print(theta)
			for i = 1,self.nInputPlane do 
			   self.weight[j][i] = gabor2(self.kW, 1,theta, 0,1.68,0.5, 1)
			   self.weight[j+1][i] = gabor2(self.kW, 1,theta, 0,1.68,0.5, 0)
  			end
 		 end
...
--function Gaborconv2:gbllr()
--if self.gblr == 0 then
--print('gblr=0, no updating')
...
print(self.gblr)
	function Gaborconv2:updateParameters(learningRate)
		local params, gradParams = self:parameters()
  		 if params then
      		for i=1,#params do
         		params[i]:add(-self.gblr, gradParams[i])
print('upgrading gbl:')
print(self.gblr)
     		 end
   		end
